...
LOG_TXT_PATH = '../logs/finetuning3W_print_0.txt'
...
EMA_DECAY      = 0.9995
MAX_NORM       = 1.0
CKPT_PATH      = '../model2/Large.pth'
SAVE_PATH      = '../model2/Large_0.pth'
...
BEST_SAVE_PATH = '../model2/Large_best0.pth'
...
# ================== 评估（默认用EMA更稳） ==================
@torch.no_grad()
def evaluate(model_for_eval, loader, device='cuda', ds_flag=0):
    model_for_eval.eval()
    correct = total = 0
    ...
    while global_step < TOTAL_ITERS:
        ...
        if micro_step % ACCUM_STEPS == 0:
            ...
            if global_step % EVAL_INTERVAL == 0 or global_step == TOTAL_ITERS:
                ...
                acc = evaluate(ema.module, test_loader, device=device, ds_flag=0)
